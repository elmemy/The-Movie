...
class MovieDetailsPresenterImplementation: MovieDetailsPresenter {
    ...
    func isFav() {
        ...
        if myFav.count == 0{
            ...
            myFavmovie.id = movie.id
            myFavmovie.title = movie.title
            myFavmovie.overview = movie.overview
            myFavmovie.posterPath = (URLs.BaseImage + (movie.posterPath ?? ""))
            myFavmovie.voteAverage = movie.voteAverage
            try! realm.write {
